...
```mermaid
flowchart TB
    Client["Multi-Tenant Clients"] --> API["Async API"]
    API --> Queue["FIFO Job Queue"]

    Queue --> Worker1["Worker"]
    Queue --> Worker2["Worker"]
    Queue --> Worker3["Worker"]

    Worker1 --> Pool["Resource Pool"]
    Worker2 --> Pool
    Worker3 --> Pool
    ...
    R1 --> Upstream["Upstream Provider"]
    R2 --> Upstream
    RN --> Upstream

    Upstream --> Result["Job Result"]
    Result --> Queue
```

The important boundary is the resource pool. Workers can scale horizontally, while the pool ensures that active upstream usage never exceeds the provider's real concurrency limit.
